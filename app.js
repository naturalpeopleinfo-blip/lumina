(function () {
  ...
  function getSpeedIconMarkup(speed) {
    if (speed === 1) {
      return (
        '<img class="speed-icon speed-icon-walk" src="./assets/walking.svg" alt="" aria-hidden="true">'
      );
    }

    return (
      '<img class="speed-icon speed-icon-run" src="./assets/running.svg" alt="" aria-hidden="true">'
    );
  }
  ...
  function buildReportFlags(flags) {
    return sortFlagsForReport(flags).map(function (flag, index) {
      ...
      return Object.assign(reportFlag, {
        ...
        timeLabel: getFlagTitle(flag),
        reportPositionLabel: getReportPositionLabel(reportFlag),
        reportPositionShort: getReportPositionBaseLabel(reportFlag),
        reportZone: reportFlag.zone,
        reportZoneLabel: FLAG_LABELS[reportFlag.zone] || FLAG_LABELS.center,
        reportZoneColor: getFlagZoneColor(reportFlag.zone),
  ...
  function getFlagZoneColor(zone) {
    return FLAG_ZONE_COLORS[zone] || FLAG_ZONE_COLORS.center;
  }
      ...
      pages.push({
        ...
        flags: (page.flags || []).map(function (flag) {
          return {
            ...
            reportZone: flag.reportZone || flag.zone || "",
            reportZoneLabel: flag.reportZoneLabel || "",
            reportZoneColor: flag.reportZoneColor || getFlagZoneColor(flag.reportZone || flag.zone || "center"),
            ...
            timeLabel: flag.timeLabel || flag.reportTitle || "",
  ...
  function buildReportSectionHtml(entry, index, totalSections) {
    ...
    var instructionHtml = entry.flags.map(function (flag, flagIndex) {
      ...
      var zoneColor = flag.reportZoneColor || getFlagZoneColor(flag.reportZone || flag.zone || "center");
      return (
        '<article class="' + itemClassName + '" data-flag-id="' + escapeHtml(flagId) + '" data-flag-index="' + escapeHtml(String(flagIndex)) + '" data-report-status="' + escapeHtml(flag.reportStatus || "pending") + '" data-preview-src="' + escapeHtml(previewFrameLookup[flagId] || entry.previewImageDataUrl || "") + '" role="button" tabindex="0" aria-current="' + (flagId === defaultActiveFlagId ? "true" : "false") + '">' +
          '<div class="report-instruction-head">' +
            '<div class="report-instruction-main">' +
              '<span class="' + noBadgeClass + '" style="background:' + escapeHtml(zoneColor) + ';">' + escapeHtml(String(flag.reportNo || "")) + "</span>" +
              "<strong>" + escapeHtml(getFlagTitle(flag)) + "</strong>" +
              '<span class="report-instruction-position" style="color:' + escapeHtml(zoneColor) + ';">（' + escapeHtml(flag.reportPositionShort || flag.reportPositionLabel || FLAG_LABELS[flag.zone]) + "）</span>" +
            "</div>" +
    ...
    var previewPinsHtml = entry.flags.map(function (flag, flagIndex) {
      ...
      return (
        '<button type="button" class="' + pinClassName + '" data-flag-id="' + escapeHtml(flagId) + '" data-flag-index="' + escapeHtml(String(flagIndex)) + '" style="left:' + escapeHtml(String((flag.x || 0) * 100)) + "%;top:" + escapeHtml(String((flag.y || 0) * 100)) + "%;background:" + escapeHtml(zoneColor) + ';">' +
