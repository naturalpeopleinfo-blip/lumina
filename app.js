(function () {
  ...
  function buildReportDownloadScript(documentTitle, reportTitle, headerMetaLine, reportPages, shareApiBase) {
    return [
      ...
      'async function copyShareUrl(){if(!copyButton){return;}var originalLabel=copyButton.textContent;copyButton.disabled=true;copyButton.textContent="発行中...";try{var sharePayload=buildSharePayload();var response=await fetch(String(shareApiBase||"").replace(/\\/+$/,"")+"/api/share-create",{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},body:JSON.stringify(sharePayload)});var payload=await safeFetchJson(response);if(!response.ok||!payload||!payload.url){var detail=(payload&&payload.detail)||(payload&&payload.error)||(payload&&payload.raw)||("HTTP "+response.status);throw new Error(detail);}await copyText(payload.url);copyButton.textContent="コピーしました";setTimeout(function(){copyButton.textContent=originalLabel;copyButton.disabled=false;},1800);return;}catch(error){console.error("Failed to create share report",error);var message=(error&&error.message)?String(error.message):"共有URLの発行に失敗しました。";window.alert("共有URLの発行に失敗しました。\\n\\n原因: "+message);copyButton.textContent=originalLabel;copyButton.disabled=false;}}',
